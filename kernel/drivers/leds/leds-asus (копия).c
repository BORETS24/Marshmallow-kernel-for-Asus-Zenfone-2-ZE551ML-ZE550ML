...
/* +++global variables+++ */
static int red_led_flag, green_led_flag, blue_led_flag, red_blink_flag, green_blink_flag, blue_blink_flag, power_on_flag;
        ...
        struct device_attribute *attr, char *buf)
{
	...
	if(!strcmp(led_dat->cdev.name, "red"))
		...
		len += sprintf(buf + len, "%d\n", green_blink_flag);
	else if(!strcmp(led_dat->cdev.name, "blue")&&(blue_blink_flag==1))
		len += sprintf(buf + len, "%d\n", blue_blink_flag);
	...
	if(disable_led_flag==0) {
		...
		if(value==0) {
			...
			if((!strcmp(led_dat->cdev.name, "red"))&&(power_on_flag==1)) {
				...
				green_blink_flag = 0;
				green_led_flag = 0;
			}else if((!strcmp(led_dat->cdev.name, "blue"))&&(power_on_flag==1)) {
				led_i2c_write(led_client, SELECT1_REG, (led_i2c_read(led_client, SELECT1_REG)&(~BLUE_BIT)));
				led_i2c_write(led_client, SELECT1_REG, (led_i2c_read(led_client, SELECT2_REG)&(~BLUE_BIT)));
				blue_blink_flag = 0;
				blue_led_flag = 0;
			}
			/*power off*/
			if((red_led_flag==0)&&(green_led_flag==0)&&(red_blink_flag==0)&&(green_blink_flag==0)&&(blue_led_flag==0)&&(blue_blink_flag==0))
			...
			if(!strcmp(led_dat->cdev.name, "red")) {
				...
				led_i2c_write(led_client, SELECT1_REG, (led_i2c_read(led_client, SELECT1_REG)|BLUE_BIT));
				led_i2c_write(led_client, SELECT2_REG, (led_i2c_read(led_client, SELECT2_REG)|BLUE_BIT));
				led_i2c_write(led_client, FADE_ON_TIME_REG, 0x40);
				led_i2c_write(led_client, FULLY_ON_TIME_REG, 0x41);
				led_i2c_write(led_client, F_FULLY_OFF_TIME_REG, 0x48);
				led_i2c_write(led_client, FADE_OFF_TIME_REG, 0x40);
				led_i2c_write(led_client, S_FULLY_OFF_TIME_REG, 0x48);
				blue_led_flag = 1;
				blue_blink_flag = 1;
			}
		}else
...
void led_brightness_set(int led, int brightness) {	//led=0:red, led=1:green
	...
	if(brightness==0) {
		if((led==0)&&(red_led_flag==1)) {
			...
			led_i2c_write(led_client, SELECT1_REG, (led_i2c_read(led_client, SELECT1_REG)&(~GREEN_BIT)));
			green_led_flag = 0;
		}else if((led==2)&&(blue_led_flag==1)) {
			led_i2c_write(led_client, SELECT1_REG, (led_i2c_read(led_client, SELECT1_REG)&(~BLUE_BIT)));
			blue_led_flag = 0;
		}
		if((red_led_flag==0)&&(green_led_flag==0)&&(blue_led_flag==0)&&(red_blink_flag==0)&&(green_blink_flag==0)&&(blue_blink_flag==0))
			led_enable_set(INDCAT_EN, 0);
